'use client';
import { motion, useAnimation, RepeatType, TargetAndTransition } from 'framer-motion';
    ...
    >
      ...
      >
        {[...items, ...items].map((item, idx) => (
          ...
          >
            <p className="text- font-catamaran text-white italic">”{item.quote}”</p>
            <span className="mt-3 font-catamaran text-white font-bold">{item.name}</span>
            <span className="text-xs font-catamaran font-white text-gray-200">{item.title}</span>
          </div>
        ))}
      </motion.div>
    </div>
